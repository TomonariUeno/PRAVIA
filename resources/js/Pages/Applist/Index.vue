<script setup>
    ...
    const columns = ref([
        ...
        { key: 'system_map_no', label: 'System Map No', visible: true, fixed: true, left: 200 },
    ...
    // 検索
    const search = () => {
        router.get(route('applist.index'), {
            ...form,
            sort: props.sort,
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="card">
                ...
                <div class="flex gap-6 mt-4">
                    <label><input type="checkbox" v-model="form.pas_use"> PAS</label>
                    <label><input type="checkbox" v-model="form.pits_use"> PITS</label>
                    <label><input type="checkbox" v-model="form.pce_use"> PCE</label>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="form-item">
                    ...
                    <div class="checkbox-group">
                        <label v-for="item in tsaExitMehotds" :key="item.value">
                ...
                <div class="form-item">
                    <label>SA方針</label>
                    <div class="checkbox-group">
                        <label v-for="item in tsaExitMehotdsPas" :key="item.value">
            ...
            <div class="card">
                ...
                <div class="form-item">
                    <div class="checkbox-group">
                        <label v-for="item in waves" :key="item.value">
            ...
            <div class="mb-4 flex gap-4">
                <button @click="exportCsv" class="bg-green-500 text-white px-4 py-2 rounded">
                    CSV出力
                </button>
            </div>
            ...
            <div class="mb-4 flex flex-wrap gap-2">
                <label v-for="col in columns" :key="col.key" class="flex items-center gap-1">
                    <input type="checkbox" v-model="col.visible" v-if="col">
                    {{ col.label }}
                </label>
            </div>
                ...
                <table class="min-w-max border">
                    <thead class="bg-gray-100">
                        <tr class="bg-gray-100">
                            <th v-for="col in columns" v-if="col && col.visible" :key="col.key" class="p-2 cursor-pointer whitespace-nowrap" :class="{ 'sticky bg-gray-100 z-10': col.fixed }" :style="col.fixed ? { left: col.left + 'px' } : {}" @click="sortBy(col.key)">
                                {{ col.label }} {{ getSortIcon(col.key) }}
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="row in applist?.data ?? []" :key="row.id">
                            <td v-for="col in columns" v-if="col && col.visible" :key="col.key" class="p-2 whitespace-nowrap" :class="{ 'sticky bg-white z-10': col.fixed }" :style="col.fixed ? { left: col.left + 'px' } : {}">
                                <template v-if="col.key === 'id'">
                                    <Link :href="route('applist.show', row.id)">
                                        {{ row.id }}
                                    </Link>
                                </template>
                                <template v-else>
